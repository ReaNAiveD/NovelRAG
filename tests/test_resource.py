...
from novelrag.resource.operation import OperationTarget, PropertyOperation, ResourceLocation, ResourceOperation
...
async def create_test_repository(*, use_mock: bool = True):
    ...
    _resource_config = {
        ...
        "event": AspectConfig(
            path="resource/events.yml", description="A collection of events in the story", children_keys=["subEvents"]
        ),
    }
    ...
    with patch(
        "novelrag.storage.local.resource.LanceDBStore.create",
        new=AsyncMock(side_effect=lambda **kwargs: DummyVectorStore(embedder)),
    ):
        return await LanceDBResourceRepository.load_from_disk(cfg_path, vector_store_config, embedder)
...
class RepositoryTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_add_elements(self):
        ...
        await self.repository.apply(
            ResourceOperation.new(
                ...
                data=[self.test_data.create_character("Alice", 25), self.test_data.create_character("Bob", 30)],
            )
        )
    ...
    async def test_modify_element(self):
        ...
        await self.repository.apply(
            ResourceOperation.new(
                location=ResourceLocation.aspect("character"), data=[self.test_data.create_character("Alice", 25)]
            )
        )
    ...
    async def test_nested_elements(self):
        ...
        await self.repository.apply(ResourceOperation.new(location=ResourceLocation.aspect("event"), data=[main_event]))
...
class ElementTreeTestCase(unittest.TestCase):
    ...
    def test_element_set_children_names(self):
        ...
        main_event = self.test_data.create_event("Main Event", ["Alice"])
        sub_events = [self.test_data.create_event(f"Sub Event {i}", [f"Character{i}"]) for i in range(1, 4)]
