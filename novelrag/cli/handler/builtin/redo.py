...
class RedoHandler(Handler):
    ...
    async def handle(self, command: Command) -> HandlerResult:
        redo_tasks = await self.undo_queue.pop_redo_group()
        if not redo_tasks:
        ...
        for task in redo_tasks:
            match task.method:
                case "apply":
                    ...
                    await self.undo_queue.add_undo_item(
                        ReversibleAction(method="apply", params={"op": redo.model_dump()}, group=task.group),
                        clear_redo=False,
                    )
                case "update_relationships":
                    ...
                    undo_relationships = await self.resource_repo.update_relationships(
                        source_uri, target_uri, relationships
                    )
                    await self.undo_queue.add_undo_item(
                        ReversibleAction(
                            ...
                            params={
                                ...
                                "relationships": undo_relationships,
                            },
                            group=task.group,
                        ),
                        clear_redo=False,
                ...
                case "add_aspect":
                    ...
                    await self.undo_queue.add_undo_item(
                        ReversibleAction(method="remove_aspect", params={"name": name}, group=task.group),
                        clear_redo=False,
                    )
                case _:
                    return HandlerResult(
        ...
        details = UndoRedoDetails(
            action="redo",
            methods=[t.method for t in redo_tasks],
            count=len(redo_tasks),
            descriptions=[t.description for t in redo_tasks],
        )
        if len(redo_tasks) == 1:
            return HandlerResult(
