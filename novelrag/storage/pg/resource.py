...
class PostgresResourceRepository(ResourceRepository):
    ...
    async def _insert_elements(
        ...
        await cur.executemany(
            ...
            [
                ...
                for e, embedding in zip(elements, embeddings, strict=True)
            ],
        )
    ...
    async def _find_element_by_uri(self, cur: AsyncCursor[DictRow], uri: str) -> ElementRow | None:
        ...
        row = await cur.fetchone()
        return (
            ElementRow(element=self._row_to_element(row), aspect_id=row["aspect_id"], element_id=row["id"])
            if row
            else None
        )
    ...
    async def _iter_elements_of_aspect(self, cur: AsyncCursor[DictRow], aspect_name: str) -> list[ElementRow]:
        ...
        return [
            ElementRow(
                element=self._row_to_element(r, aspect_name=aspect_name), aspect_id=r["aspect_id"], element_id=r["id"]
            )
            for r in rows
        ]
        ...
        async with self.pool.connection() as conn, conn.transaction(), conn.cursor(row_factory=dict_row) as cur:
            ...
            if isinstance(target, list):
                ...
                if op.location.children_key is not None:
                    logger.warning(
                        f"ResourceOperation location for aspect '{aspect.name}' should not specify children_key, but got '{op.location.children_key}'. Ignoring children_key."
                    )
                new_names, undo_data = await self._splice_children(
                    cur,
                    target.aspect_id,
                    op.location.resource_uri,
                    aspect.name,
                    aspect.children_keys or [],
                    aspect.root_element_names,
                    op,
                )
                ...
                if op.location.children_key is None:
                    raise ValueError(
                        f"ResourceOperation location for element '{element.uri}' must specify children_key."
                    )
                new_names, undo_data = await self._splice_children(
                    cur,
                    target.aspect_id,
                    element.uri,
                    element.aspect,
                    element.children_keys or [],
                    element.children_names_of(op.location.children_key),
                    op,
                )
                element.set_children_names(op.location.children_key, new_names)
                await conn.execute(
                    "UPDATE resource_elements SET data = %(data)s WHERE workspace_id = %(ws)s AND uri = %(uri)s",
                    {
                        "ws": self.workspace_id,
                        "uri": element.uri,
                        "data": element.model_extra,
