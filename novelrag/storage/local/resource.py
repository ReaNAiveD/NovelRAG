...
from novelrag.resource.repository import RemovedAspectResult, ResourceRepository, SearchResult
...
class LanceDBResourceRepository(ResourceRepository):
    ...
    * ``Element.children_names`` — ordered child-name lists per
    ...
    async def load_from_disk(
        ...
        with open(aspects_config_path, encoding="utf-8") as f:
        ...
        if vector_store_config.cleanup_invalid_on_init:
            invalid_count = await vector_store.cleanup_invalid_resources(
                valid_uris=set(ele.uri for ele in all_elements)
            )
            if invalid_count > 0:
    ...
    async def _load_aspect_from_disk(name: str, aspect: AspectConfig) -> tuple[ResourceAspect, list[Element]]:
        ...
        if not os.path.exists(aspect.path):
            logger.warning(
                f"Aspect file for aspect '{name}' not found at path '{aspect.path}'. Initializing with empty root elements."
            )
            aspect_obj = ResourceAspect.from_config(name, aspect, root_element_names=[])
            return aspect_obj, []
        with open(aspect.path, encoding="utf-8") as f:
    ...
    async def _dump_with_uri(self, uri: str):
        ...
        if uri == "/":
            ...
            for aspect_name in self.aspect_configs:
    ...
    async def vector_search(
        self, query: str, *, aspect: str | None = None, limit: int | None = 20
    ) -> list[SearchResult]:
    ...
    async def apply(self, op: Operation) -> Operation:
        ...
        if isinstance(op, PropertyOperation):
            ...
            if not aspect_obj:
                raise ValueError(
                    f"Aspect '{aspect_name}' not found for ResourceOperation at URI '{op.location.resource_uri}'."
                )
            ck = aspect_obj.children_keys
            new_direct, new_descendants = load_elements(
                op.data or [],
                parent_uri=op.location.resource_uri,
                aspect=aspect_name,
                children_keys=ck,
            )
            ...
            if isinstance(target, Element):
                if op.location.children_key is None:
                    raise ValueError(
                        "ResourceOperation with target 'resource' must specify a children_key for element targets."
                    )
                ...
                target.set_children_names(
                    op.location.children_key,
                    current_children[: op.start] + new_children_names + current_children[op.end :],
                )
            elif isinstance(target, ResourceAspect):
                if op.location.children_key is not None:
                    logger.warning(
                        f"ResourceOperation with target 'resource' on aspect '{target.name}' should not specify a children_key. Ignoring children_key '{op.location.children_key}'."
                    )
            ...
            for child_uri in undo_uris:
                if child_uri not in self.lut.table:
                    logger.warning(f"Element with URI '{child_uri}' not found in lookup table.")
                    continue
