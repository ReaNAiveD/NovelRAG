...
def validate_op_json(op: str) -> Operation:
    return TypeAdapter(Operation).validate_json(op)


class ObjectLocation:
    def __init__(self, parent: dict[str, Any] | list | None, idx: str | None, loc: list[str]):
        self.parent = parent
        self.idx = idx
        self.loc = loc
    ...
    def target(self):
        ...
        if isinstance(self.parent, dict):
            ...
            try:
                ...
                raise KeyError(
                    f"Key '{self.idx}' of Path {_format_path(self.loc + [self.idx])} is not Allowed for a list."
                ) from e
        else:
