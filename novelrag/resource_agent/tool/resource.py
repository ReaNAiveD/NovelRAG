...
class ResourceWriteTool(SchematicTool):
    ...
    def __init__(
        ...
        content_proposers: list[ContentProposer] = [
            LLMContentProposer(chat_llm=chat_llm, lang=lang, lang_directive=lang_directive)
        ]
    ...
    def description(self):
        return (
            "Use this tool to modify existing resources within established aspects. "
    ...
    async def call(self, ctx: ExecutionContext, **kwargs) -> ToolOutput:
        ...
        if not operation_specification:
            return self.error(
                "No operation specification provided. Please provide a detailed description of the operation to perform."
            )
        content_generation_tasks = kwargs.get("content_generation_tasks", [])
        if not content_generation_tasks:
            return self.error(
                "No content generation tasks provided. Please provide at least one content generation task."
            )
        ...
        try:
            operations = await self.build_operations(
                action=operation_specification, context=await self.context.snapshot(), content_results=content_results
            )
    ...
    async def build_operations(
        self, action: str, context: ContextSnapshot, content_results: list[dict] | None = None
    ) -> list[dict]:
