...
class ResourceRelationWriteTool(SchematicTool):
    ...
    def description(self) -> str:
        return (
            "This tool is used to write relations between resources in the repository. "
    ...
    async def call(self, ctx: ExecutionContext, **kwargs) -> ToolOutput:
        ...
        if not source_resource_uri or not target_resource_uri or not operation or not relation_description:
            await ctx.error(
                "Missing required parameters: source_resource_uri, target_resource_uri, operation, relation_description."
            )
            return self.error(
                "Missing required parameters: source_resource_uri, target_resource_uri, operation, relation_description."
            )
        ...
        if isinstance(target_resource, list):
            await ctx.error(
                f"Target resource URI '{target_resource_uri}' points to multiple resources. Please specify a single resource."
            )
            return self.error(
                f"Target resource URI '{target_resource_uri}' points to multiple resources. Please specify a single resource."
            )
        ...
        if isinstance(source_resource, Element):
            ...
            undo_relationships = await self.repo.update_relationships(
                source_resource_uri, target_resource_uri, new_source_to_target
            )
            ...
            await ctx.output(
                f"Updated relations for source resource '{source_resource_uri}' to target resource '{target_resource_uri}'."
            )
            ...
            if isinstance(target_resource, Element):
                ...
                undo_relationships = await self.repo.update_relationships(
                    target_resource_uri, source_resource_uri, new_target_to_source
                )
                if self.undo is not None:
                    await self.undo.add_undo_item(
                        ReversibleAction(
                ...
                await ctx.output(
                    f"Updated relations for target resource '{target_resource_uri}' to source resource '{source_resource_uri}'."
                )
            ...
            await ctx.error(
                f"Source resource URI '{source_resource_uri}' does not point to a valid resource. Please check the URI."
            )
            return self.error(
                f"Source resource URI '{source_resource_uri}' does not point to a valid resource. Please check the URI."
            )
    ...
    async def get_updated_relations(
        self,
        source_resource: Element,
        target_resource: Element | ResourceAspect,
        existing_relation: list[str],
        operation: str,
        relation_description: str,
    ) -> list[str]:
