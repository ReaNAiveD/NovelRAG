...
from novelrag.config.llm import ChatConfig, EmbeddingConfig
...
class NovelRagConfig(BaseModel):
    ...
    agent_beliefs: Annotated[
        list[str], Field(description="List of beliefs/constraints for the agent", default_factory=list)
    ]
    default_resource_dir: Annotated[
        str, Field(description="Default directory for resources of new aspect", default=".")
    ]
    ...
    def _migrate_template_lang(cls, data):
        ...
        if isinstance(data, dict) and "template_lang" in data:
            if "language" not in data or data["language"] is None:
                ...
                _logger.warning("Config field 'template_lang' is deprecated. Please use 'language' instead.")
            else:
